...
namespace Rivet {
  ...
  class ATLAS_2015_ttJetsComm : public Analysis {
  public:
    
    ATLAS_2015_ttJetsComm() : Analysis("ATLAS_2015_ttJetsComm"){ }
    ...
    void initializeHistos() {
      ...
      // Total events histograms
      // TODO: just replace each of these with one number (at the moment, all the bins have the same value)
      //   ->: at first, it would have been simpler to directly divide the two histograms with the "efficiency" method from YODA
      //   ->: but since the P_TQ have to be integrated anyway, we cannot get rid of the Scatter2Ds
	...
	_h_neventsMQ01->point(j).setY(eventsM1, sqrt(eventsM1err));
      ...
      // Construct the jetEvents Scatter2Ds by integrating the p_TQs
      ...
      // Compute the gap fraction and set the value in the final plot
      ...
      integrateInv(_h_jetEventsMQsum1, _h_pTMQsum1);
      integrateInv(_h_jetEventsMQsum2, _h_pTMQsum2);
  ...
  private:
    ...
    // Integrate the source histogram (here the P_TQs) from bin i to the end,
    // and set the corresponding bin i in the target Scatter2D (here the 
    // jetEvents(M)Q[0,sum][1-4]
    ...
    // Calculate the ratio of the accepted (jetEvents) and the total (nevents) Scatter2D,
    // and set 1-ratio in the corresponding gap fraction Scatter2D
  ...
  DECLARE_RIVET_PLUGIN(ATLAS_2015_ttJetsComm);
}
