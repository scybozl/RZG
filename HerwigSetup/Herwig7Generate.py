...
nEvPerFile = 10
nRuns = 10
newMerge = True		# Do you want to re-merge the YODA files in the end?
newControl = True	# Do you want to check the number of events per file again?
ControlIndex = ""	# User hook to select only part of the settings in the options file
EnergyIndex = ""

WorkFolder        = "/afs/ipp-garching.mpg.de/home/l/lscyboz/HerwigSetup/"
HerwigPath	  = WorkFolder+"../Herwig-7.0.3/"
RivetAnas	  = WorkFolder+"../RivetCustomAnalyses/"

fUser = os.getenv("USER")
SettingsFolder    = WorkFolder+"Settings/"
SetupFileNameGen    = "setupfile.in"

# Folder containing the numbered subfolders with parameter values for tuning
pars 		= WorkFolder+"scan/"
...
#                  ]
    ...
    setupLines = ["source "+HerwigPath+"bin/activate\n",
		  "source "+HerwigPath+"src/Rivet-2.4.0/rivetenv.sh\n",
		  "export RIVET_ANALYSIS_PATH="+RivetAnas+":$RIVET_ANALYSIS_PATH\n"]
...
#    tmpFolder        = sampledPars+specStr+"/"
    ...
    tmp              = "$TMPDIR/"+fUser+"/"+settings+"_"+specStr+"/"
...
os.system("source "+HerwigPath+"src/Rivet-2.4.0/rivetenv.sh")
os.system("export RIVET_ANALYSIS_PATH="+RivetAnas+":$RIVET_ANALYSIS_PATH")
	  ...
	  for e,energies in enumerate(options[1].split("\t")):
	    ...
	    for scales in options[2].split("\t"):
			...
			topmass=topmasses
	                ...
	                sampledPars = WorkFolder+"MC_Herwig_"+settings+"/"
	                ...
	                ## is needed, just check if one of the final yoda files exists.
	                breakLoop = False 
	                if os.path.exists(sampledPars+"MC_Herwig_"+settings+"_"+options[index+1].split("\t")[0]+".yoda") and newControl == False or breakLoop: break
	                if order=="LO":
	                          InputFolder=WorkFolder+"GenericLO/"
	                elif order=="NLO":
	                          InputFolder=WorkFolder+"Generic/"
